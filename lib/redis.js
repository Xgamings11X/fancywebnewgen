...
export const OrdersAsync = {
  ...
  async purchaseCount(username, productId, scope = 'per_product', categoryId = null) {
    const ords = (await OrdersAsync.all()).filter(o =>
      ['success','paid','settlement','capture'].includes(o.payment_status) &&
      String(o.player_username || '').toLowerCase() === String(username || '').toLowerCase());
...
// ── Authenticated player feedback ───────────────────────────────
export const FeedbackAsync = {
  async all() {
    if (!hasDB()) {
      return (readData('feedback.json') ?? [])
        .slice()
    ...
    const pool = getPool();
    try {
      const [rows] = await pool.query('SELECT id, data, created_at, updated_at FROM feedback ORDER BY updated_at DESC');
      return rows.map(row => {
      ...
      console.error('[db] feedback.all:', error.message);
      return readData('feedback.json') ?? [];
    }
  },
  ...
  async recent(limit = 8) {
    ...
    return (await FeedbackAsync.all()).slice(0, safeLimit);
  },

  async upsert(feedback) {
    const now = new Date().toISOString();
    if (!hasDB()) {
      const all = readData('feedback.json') ?? [];
      const index = all.findIndex(item => String(item.username).toLowerCase() === String(feedback.username).toLowerCase());
      const previous = index >= 0 ? all[index] : null;
      const saved = {
        ...previous,
        ...feedback,
        id: previous?.id || `feedback-${Date.now()}`,
        created_at: previous?.created_at || now,
        updated_at: now,
      };
      if (index >= 0) all[index] = saved;
      else all.push(saved);
      writeData('feedback.json', all);
      return saved;
    }

    await ensureTables();
    const pool = getPool();
    const data = { ...feedback, updated_at: now };
    await pool.query(
      'INSERT INTO feedback (username, data) VALUES (?, ?) ON DUPLICATE KEY UPDATE data = VALUES(data), updated_at = CURRENT_TIMESTAMP',
      [feedback.username, JSON.stringify(data)]
    );
    const [rows] = await pool.query('SELECT id, data, created_at, updated_at FROM feedback WHERE username = ?', [feedback.username]);
    const row = rows[0];
    const stored = typeof row.data === 'string' ? JSON.parse(row.data) : row.data;
    return { ...stored, id: row.id, created_at: stored.created_at || row.created_at, updated_at: stored.updated_at || row.updated_at };
  },
};
