 ...
 *
 * Embed testimoni transaksi untuk channel player: ringkas dan berbasis teks.
...
function buildPremiumEmbed(order, cfg) {
  ...
  const fields = [
    { name: '**PEMBELI**', value: `\`${order.buyer_username || order.player_username || '-'}\``, inline: true },
    { name: '**PENERIMA**', value: `\`${order.player_username || '-'}\``, inline: true },
    ...
    { name: '**STATUS**',   value: `**${cfg.label}**`,                         inline: false },
  ];
  ...
  return {
    ...
    description: `## ${cfg.labelId}\nTerima kasih sudah bertransaksi di **Fancy Network**. Pesanan telah tercatat dan diproses untuk akun Minecraft penerima.`,
    fields,
...
export async function webhookTransaction(order) {
  ...
  if (isSuccess && playerUrl) {
    ...
    const customerName  = order.discord_username || order.buyer_username || order.player_username || '-';
    const playerContent = `Transaksi **${customerName}** berhasil diproses oleh **Fancy Network**.`;
